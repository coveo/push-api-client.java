...
class DocumentUploadQueue {
   ...
   * Used by getConfiguredBatchSize and constructors to ensure consistent validation logic.
   * 
   ...
   * 
   * Uses the configured batch size from system property "coveo.push.batchSize" if set,
   ...
   *
   * @param uploader The upload strategy to be used for document uploads.
   * @param maxQueueSize The maximum queue size in bytes (e.g., 52428800 for 50 MB). Must not exceed 256MB (Stream API limit).
